...
object GUI_View extends JFXApp {
  ...
  stage = new JFXApp.PrimaryStage {
    title.value = "КиТПО ЛР-2. Односвязный список."
    width = fullScreenX
    height = fullScreenY
    centerOnScreen()
    resizable = false
    ...
    scene = new Scene {
      ...
      val load_menu = new MenuItem("Загрузить из файла")
      load_menu.accelerator = new KeyCodeCombination(KeyCode.I,KeyCombination.ControlDown)
      load_menu.onAction = (e:ActionEvent) => {
        try {
          var i:ObjectInputStream = new ObjectInputStream(new FileInputStream("list.txt"))
      ...
      val save_menu = new MenuItem("Сохранить в файл")
      save_menu.accelerator = new KeyCodeCombination(KeyCode.O,KeyCombination.ControlDown)
      save_menu.onAction = (e:ActionEvent) => {
        try{
          var out:ObjectOutputStream = new ObjectOutputStream(new FileOutputStream("list.txt"))
      ...
      textArea.layoutX = 25
      textArea.layoutY = PossYInsert
      textArea.prefHeight = fullScreenY-2.2*PossYInsert
      textArea.prefWidth = 3*fullScreenX/5
      textArea.editable = false

      val PossX = 3*fullScreenX/5 + 50

      val changeTypeLabel = new Label("Выберете тип данных")
      changeTypeLabel.layoutX = PossX
      changeTypeLabel.layoutY = 50
      ...
      pushBack_button.layoutX = PossX+200
      pushBack_button.layoutY = 160
      ...
      insertToIndexLabel.layoutY = 200

      val PossYInsertToIndex = 225
      ...
      deleteLabel.layoutY = 270

      val PossYDelete = 270
      val PossYDelete1 = 310

      val delFront = new Button("Первый")
      delFront.layoutX = PossX + 60
      ...
      val delBack = new Button("Последний")
      delBack.layoutX = PossX+160
      ...
      deleteTextFieldTwo.layoutX = PossX+70
